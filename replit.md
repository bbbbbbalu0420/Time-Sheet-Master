...
- **Output file**: Hours written to col2 of day pair, green fill (FF90EE90) for work hours, yellow (FFFFFF00) for vacation/sick/unpaid leave, red (FFFF6B6B) for fired. IMPORTANT: ExcelJS shared cell styles require using `cell.style = { ...cell.style, fill }` instead of `cell.fill = fill` to break shared style references when overwriting fills.
- **Employee row totals**: After writing daily hours, BM (col 65), BN (col 66), BO (col 67) are written as values (not formulas). With clearHours: BM=0, BN=totalHours, BO=totalHours.
- **Objects table update**: Bottom table (stores/objects) is automatically updated for the new month. Left-side formulas (col O/F referencing employee BM/BN) are evaluated using employee total hours and written as values. Right-side static hours (C22/C23) are scaled proportionally using the ratio of January-to-December employee totals (only employees with known December cached BO values are used for accurate scaling). SUM and composite formulas (rows 42-43) are re-evaluated after individual store formulas.
